...
?>
<span class="back"><a href="<?php echo tribe_get_events_link(); ?>"><?php _e('&laquo; Tüm Etkinlikler', 'tribe-events-calendar'); ?></a></span>				
<?php
	$gmt_offset = (get_option('gmt_offset') >= '0' ) ? ' +' . get_option('gmt_offset') : " " . get_option('gmt_offset');
 	$gmt_offset = str_replace( array( '.25', '.5', '.75' ), array( ':15', ':30', ':45' ), $gmt_offset );
 	if (strtotime( tribe_get_end_date(get_the_ID(), false, 'Y-m-d G:i') . $gmt_offset ) <= time() ) { ?><div class="event-passed"><?php  _e('Bu etkinliğin tarihi geçmiştir.', 'tribe-events-calendar'); ?></div><?php } ?>
<div id="tribe-events-event-meta" itemscope itemtype="http://schema.org/Event">
	<dl class="column">
		<dt class="event-label event-label-name"><?php _e('Etkinlik:', 'tribe-events-calendar'); ?></dt>
		<dd itemprop="name" class="event-meta event-meta-name"><span class="summary"><?php the_title(); ?></span></dd>
		<?php if (tribe_get_start_date() !== tribe_get_end_date() ) { ?>
			<dt class="event-label event-label-start"><?php _e('Başlama:', 'tribe-events-calendar'); ?></dt> 
			...
			<dt class="event-label event-label-end"><?php _e('Bitiş:', 'tribe-events-calendar'); ?></dt>
		...
		<?php } else { ?>
			<dt class="event-label event-label-date"><?php _e('Tarih:', 'tribe-events-calendar'); ?></dt> 
		...
		<?php if ( tribe_get_cost() ) : ?>
			<dt class="event-label event-label-cost"><?php _e('Ücret:', 'tribe-events-calendar'); ?></dt>
		...
		<?php if ( tribe_get_organizer_link( get_the_ID(), false, false ) ) : ?>
			<dt class="event-label event-label-organizer"><?php _e('Organize:', 'tribe-events-calendar'); ?></dt>
      ...
      <?php elseif (tribe_get_organizer()): ?>
		...
		<?php if ( tribe_get_organizer_phone() ) : ?>
			<dt class="event-label event-label-organizer-phone"><?php _e('Telefon:', 'tribe-events-calendar'); ?></dt>
		...
		<?php if ( class_exists('TribeEventsRecurrenceMeta') && function_exists('tribe_get_recurrence_text') && tribe_is_recurring_event() ) : ?>
			<dt class="event-label event-label-schedule"><?php _e('Takvim:', 'tribe-events-calendar'); ?></dt>
	...
	<dl class="column" itemprop="location" itemscope itemtype="http://schema.org/Place">
		...
		<dt class="event-label event-label-venue"><?php _e('Yer:', 'tribe-events-calendar'); ?></dt> 
		...
		<dt class="event-label event-label-venue-phone"><?php _e('Telefon:', 'tribe-events-calendar'); ?></dt> 
		...
		<dt class="event-label event-label-address">
			<?php _e('Adres:', 'tribe-events-calendar') ?><br />
			<?php if( tribe_show_google_map_link( get_the_ID() ) ) : ?>
				<a class="gmap" itemprop="maps" href="<?php echo tribe_get_map_link(); ?>" title="<?php _e('Google Haritada göster', 'tribe-events-calendar'); ?>" target="_blank"><?php _e('Google Harita', 'tribe-events-calendar' ); ?></a>
			<?php endif; ?>
		</dt>
...
<div class="entry-content clearfix">
	...
	if ( function_exists('has_post_thumbnail') && has_post_thumbnail() ) {?>
		<div class="pull-left" style="margin-right: 10px;"><?php the_post_thumbnail(); ?></div>
	<?php } ?>
